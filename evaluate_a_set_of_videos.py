...
mean, std = (torch.FloatTensor([123.675, 116.28, 103.53]), 
...
def rescale_results(results: list, vname="undefined"):
    dbs = {"livevqc": "LIVE_VQC", "kv1k": "KoNViD-1k", "ltest": "LSVQ_Test", "l1080p": "LSVQ_1080P",  "ytugc": "YouTube_UGC"}
...
if __name__ == "__main__":
    ...
    for (root,dirs,files) in os.walk(args.video_dir, topdown=True):
        for file in files:
    ...
    for stype, sopt in dopt["sample_types"].items():
        if "t_frag" not in sopt:
            ...
            temporal_samplers[stype] = UnifiedFrameSampler(sopt["clip_len"] // sopt["t_frag"], sopt["t_frag"], sopt["frame_interval"], sopt["num_clips"])
            
            
    # automatically no parallel computing
    for video_path in tqdm(video_paths):
        video_reader = decord.VideoReader(video_path)


        ### View Decomposition
        views, _ = spatial_temporal_view_decomposition(video_path, 
                                                    dopt["sample_types"],
                                                    temporal_samplers)

        for k, v in views.items():
            num_clips = dopt["sample_types"][k].get("num_clips", 1)
            views[k] = ((v.permute(1, 2, 3, 0) - mean) / std).permute(3, 0, 1, 2).reshape(v.shape[0], num_clips, -1, *v.shape[2:]).transpose(0,1).to(args.device)

        results = [r.mean().item() for r in evaluator(views)]
        all_results[video_path] = {"technical": results[0], "aesthetic": results[1]}
     
        with open(f"dover_predictions/val-custom_{args.video_dir.split('/')[-1]}.pkl","wb") as wf:
