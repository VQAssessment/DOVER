...
def main():
    ...
    parser.add_argument(
        "-o", "--opt", type=str, default="divide.yml", help="the option file"
    )

    parser.add_argument(
        "-t", "--train", type=str, default="train-dividemaxwell", help="training set"
    )
    
    parser.add_argument(
        "-v", "--val", type=str, default="val-dividemaxwell", help="training set"
    )
    ...
    if opt.get("split_seed", -1) > 0:
        ...
        print("No random splits, follow default")
        num_splits = 1
    ...
    for split in range(num_splits):
        ...
        run = wandb.init(
            ...
            name=opt["name"] + f"_divide_maxwell_train_split"
            if num_splits > 1
            else opt["name"],
            reinit=True,
        ...
        for epoch in range(opt["l_num_epochs"]):
            ...
            for key in val_loaders:
                bests[key] = inference_set(
                    ...
                    save_name=opt["name"] + "_head_" + args.train + f"_{split}",
                    suffix=key + "_s",
                )
                if model_ema is not None:
